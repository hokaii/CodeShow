"""
website:https://blog.csdn.net/Marksinoberg/article/details/66969620?utm_source=blogxgwz1
"""
import poplib
...
def get_parsed_msg():
    useracount='13059548599@163.com'
    password='hoka506'
    ...
    #可选项:打印欢迎文字,验证是否正确连接到服务器
    print(server.getwelcome().decode('utf-8'))
    #开始进行身份验证
    server.user(useracount)
    server.pass_(password)
    #使用list返回所有邮件的编号,默认为字节类型的串
    ...
    print("该邮件所占字节大小: {}".format(octets))
    msg_content=b'\r\n'.join(mail_message_lines).decode('gbk')
    #邮件原始数据没法正常浏览,因此需要进行相应的解码操作
...
def decode_base64(s,charset='utf-8'):
...
def get_parsed_msg():
    # 邮箱个人信息
    useraccount = '13059548599@163.com'
    password = 'hoka506'
    # 邮件服务器地址
    pop3_server = 'pop.163.com'
    # 开始连接到服务器
    ...
    #print('解码后的邮件信息:\n{}'.format(msg))
    # 关闭与服务器的连接，释放资源
    mail_message_lines=str(mail_message_lines)
    message=mail_message_lines.split('b\'')[-1]
    messagee=decode_base64(message)
    print(decode_base64(message))
    print(messagee)
    #msgg=str(msg)
    #print(msgg)
"""
