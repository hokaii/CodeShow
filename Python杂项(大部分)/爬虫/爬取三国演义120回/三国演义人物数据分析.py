...
class anylysisThreeKindoms():
    ...
    def makeWordCloud(self, book, picture):
        ...
        wc = WordCloud(background_color='white', max_words=2000, mask=picWithColor, stopwords=STOPWORDS,
                       max_font_size=40, random_state=42, font_path='simhei.ttf')
    ...
    def appearEachEpisode(self, readBook, writeFile):   # 没回中人物出现次数
    ...
    def findMostWords(self, dict, readBook, writeFile):
        ...
        with codecs.open(readBook, 'r', 'ISO-8859-1') as f:
            i = 0
            for line in f.readlines():
                self.processBar(i, 120)
                i += 1
                poss = pseg.cut(line)   # 返回每个词及其词性
                for w in poss:
                    if w.flag != 'nr' or len(w.word) < 2:
                        continue
                    if self.nouns.get(w.word) is None:
                        self.nouns[w.word] = 0
                    self.nouns[w.word] += 1     # 如果出现则次数加1
    ...
    def occurrencesTop10(self, file, html):     # 截取了出现20次以上
        ...
        bar = Bar(html)
        bar.add('出场次数', list(self.Top10['人物'])[:10], list(self.Top10['出现次数'])[:10],
    ...
    def nodeAndRelationship(self, readBook):
        ...
        with open('edge.csv', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    anylysisThreeKindoms.findMostWords('dict.txt', 'sanguo.txt', 'wordRootList.csv')
    ...
    anylysisThreeKindoms.makeWordCloud('sanguo.txt', 'test.jpg')
